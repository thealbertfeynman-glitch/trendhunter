...
Crawl video data from TikTok user profiles using Puppeteer with stealth mode. Automatically collects videos posted within the last N days along with detailed metadata.

## Part of TrendHunter Pipeline

This crawler is the data collection layer for **TrendHunter** — the #1 Trend Detection Agent. It feeds `summary.json` into the TrendHunter dashboard for trend analysis.

```
[TikTok Crawler] --> summary.json --> [TrendHunter Dashboard]
                                          |
                                          |--> Trend Groups (AI-classified)
                                          |--> Hashtag Rankings
...
output/
  ...
  username2.json
  summary.json        # Combined results --> copy to data/summary.json for TrendHunter
  report.txt          # Human-readable text report
```

## Integration with TrendHunter

After crawling, copy the output to TrendHunter:

```bash
cp output/summary.json ../../data/summary.json
```

Then restart the TrendHunter dashboard to see updated trends.
